...
function Exchange(props) {
  ...
  function formatDateTime(date) {
    ...
    // Construct the formatted date-time string
    return `${monthStr}/${dayStr}/${year} ${hourStr}:${minuteStr}`;
  }
  ...
  const handleSubmitPost = async (event) => {
    event.preventDefault();
    const titleWords = preprocessText(newPostTitle);
    const textWords = preprocessText(newPostContent);
    const postsRef = collection(firestore, 'posts');
    const newImageURL = await uploadImage();
    if (newPostTitle !== "" && newPostSubreddit !== "" && newPostContent !== "") {
      try {
        await addDoc(postsRef, {
          ...
          text: newPostContent,
          title: newPostTitle,
          titleWords, // Store preprocessed title
          textWords, // Store preprocessed text
          timestamp: Timestamp.fromDate(new Date()),
          uid: currentUID,
          upvotes: [],
        });
          ...
          posts.map((obj) => {
            return (
              <ForumPostCard
                ...
                title={obj.title}
                text={obj.text}
                timestamp={formatDateTime(new Date(obj.timestamp.seconds*1000))}
                uid={obj.uid}
                upvotes={obj.upvotes}
              />
            )
